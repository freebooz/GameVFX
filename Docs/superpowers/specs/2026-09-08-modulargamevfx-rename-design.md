...
核心插件继续只包含可迁移的 Niagara、纹理、材质、网格体、可复用模块、性能配置和源文件。演示插件继续包含角色、动画、敌人 AI、第三人称输入、施法逻辑和 `L_ModularGameVFX_Demo`，并单向依赖核心插件。
...
7. 重建模块，执行依赖审计、当前 13 个法术系统编译检查和 `L_ModularGameVFX_Demo` 运行检查。
...
- 虚幻编辑器能够加载 `L_ModularGameVFX_Demo`，玩家、怪物、输入和 1–8 号法术仍可运行。
...
- 核心插件可以独立安装到空白内容工程；启用演示插件后可以打开 `L_ModularGameVFX_Demo`。
- 发布清单中的所有文件哈希与实际文件一致。
