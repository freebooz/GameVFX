...
if(Layer==1){ // longitudinal curved flame envelope
 float roll=t*2.1;q=float4(sin(roll*.5),0,0,cos(roll*.5));color=1;alpha=.8;sc=float3(1,1+sin(t*15)*.06,1+cos(t*13)*.06);
}
...
if(Layer==33){float life=saturate((5.4-t)/1.8);float rad=35+190*s2;float theta=a+t*.28;pos=float3(cos(theta)*rad,sin(theta)*rad,40+p*160);size=float2(110+s3*65,180+s3*115)*( .6+.5*sin(p*3.14159));rot=sin(t*2+a)*20;alpha=sin(p*3.14159)*life*.85;color=1;}
...
Particles.SpriteFacing=float3(0,0,1);Particles.SpriteAlignment=float3(1,0,0);
