...
for k,v in {'texture_target':rt,'capture_source':U.SceneCaptureSource.SCS_FINAL_COLOR_LDR,'capture_every_frame':False,'capture_on_movement':False,'always_persist_rendering_state':True,'fov_angle':63.}.items():c.set_editor_property(k,v)
...
out=pathlib.Path('F:/game/MythicVFXLab/Saved/VFXValidation/FireMage/FilmFrames_Corrected');out.mkdir(parents=True,exist_ok=True)
...
def tick(dt):
    try:
        f=state['frame']
        if f==0:p.cast_fireball()
        if f==80:p.cast_fire_blast()
        if f==90:m.set_editor_property('health',100.);p.cast_flamestrike()
        c.capture_scene()
        ...
        if state['frame']>=240:
            state['finished']=True;U.SystemLibrary.execute_console_command(w,'t.OverrideFPS 0');U.unregister_slate_post_tick_callback(handle);(out/'capture.json').write_text(json.dumps(state,indent=2));print('FIRE_FILM_COMPLETE')
    except Exception:
