...
p.set_actor_location(U.Vector(0,0,96.15),False,False);p.set_actor_rotation(U.Rotator(),False);m.set_actor_location(U.Vector(1100,0,122.15),False,False);m.set_editor_property('bCombatEnabled',False);m.character_movement.disable_movement();p.select_next_target()
pos=U.Vector(180,-1120,510);look=U.Vector(540,0,145);cap.set_actor_location_and_rotation(pos,U.MathLibrary.find_look_at_rotation(pos,look),False,False)
out=pathlib.Path('F:/game/MythicVFXLab/Saved/VFXValidation/ArcaneMage/FilmFrames');out.mkdir(parents=True,exist_ok=True)
